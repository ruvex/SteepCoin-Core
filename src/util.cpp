...
const char * const BITCOIN_CONF_FILENAME = "steepcoin.conf";
const char * const BITCOIN_PID_FILENAME = "steepcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "steepcoin" is a composite category enabling all steepcoin-related debug output
                if(ptrCategory->count(std::string("steepcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\steepcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\steepcoinCore
    // Mac: ~/Library/Application Support/steepcoinCore
    // Unix: ~/.steepcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "steepcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/steepcoinCore";
#else
    // Unix
    return pathRet / ".steepcoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty steepcoin.conf if it does not excist
