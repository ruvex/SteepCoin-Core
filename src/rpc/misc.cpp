...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total steepcoin balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized steepcoin balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "                       mempoolrej, net, proxy, prune, qt, rand, reindex, rpc, selectcoins, tor, zmq, steepcoin\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "steepcoin")
            + HelpExampleRpc("debug", "steepcoin+net")
        );
...
#ifdef ENABLE_WALLET
        // Case 1: steepcoin address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are steepcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) steepcoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* steepcoin features */
    { "steepcoin",               "mnsync",                 &mnsync,                 true,  {} },
    { "steepcoin",               "spork",                  &spork,                  true,  {"value"} },
