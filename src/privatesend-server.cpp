...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all steepcoin related functionality
...
/*
    ...
    allow endless transaction that would bloat steepcoin and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all steepcoin specific functionality
