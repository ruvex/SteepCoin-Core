...
export BUILDER_IMAGE_NAME="steepcoin-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/steepcoin"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_steepcoin_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
