...
# steepcoin_hash
RUN git clone https://github.com/ruvex/SteepCoin-Core_hash
RUN cd steepcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} steepcoin
RUN useradd -u ${USER_ID} -g steepcoin -s /bin/bash -m -d /steepcoin steepcoin
...
RUN mkdir /steepcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /steepcoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /steepcoin-src
