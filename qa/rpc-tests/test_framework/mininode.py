...
def steepcoinhash(s):
    return steepcoin_hash.getPoWHash(s)
...
class CBlockHeader(object):
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(steepcoinhash(r))
            self.hash = encode(steepcoinhash(r)[::-1], 'hex_codec').decode('ascii')
